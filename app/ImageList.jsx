...
const ImageList = ({imagesData, imgStyle}) => {
    ...
    return (
        <>
        <Text>{isGameOver ? "You WIN, all cards matched" : matchedImg.length > 0 ? `You have mactched ${matchedImg.length} cards` : "Press on image to play"} </Text>
            ...
            renderItem={({item, index}) => (
                <View>
                    <Pressable onPress={() =>{
                        ...
                        <Image source={ imgSide[index] ? item : questionMark } style={imgStyle}/>
                    </Pressable>
                </View>
            )}
        /></>
